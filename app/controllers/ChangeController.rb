class ChangeController < ApplicationController
    def self.n_change(number)
        begin
            # intか判断 true or false
            n_result = !! (number.to_s =~ /^[0-9]+$/)

            # 漢字->数字
            if n_result === false

                # 単位と数字ごとに分けた配列を作成
                change_number = number.to_s.gsub(/[一|二|三|四|五|六|七|八|九]/,
            ...
            ## 数字->漢字
                ...
                c_characters_number += numbers_array[8] === "0" || numbers_array[8] === nil  ? "億" : numbers_array[8].to_s + "億"
                ...
                c_characters_number += numbers_array[4] === "0" || numbers_array[4] === nil  ? "万" : numbers_array[4].to_s + "万"
